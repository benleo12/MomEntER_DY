...
if int(_os.environ.get('EXPORT_VARS','0')):
    ...
    out={'description':'Per-scheme MaxEnt reweighting (central + Wan-Li scale/NP variations). For scheme S, per event: w_rew = w0*exp(sum_k schemes[S].lambda_physical[k]*phi_k - schemes[S].log_norm_shift); then the SAME gating as the central file. phi_k and gating identical to lambda_export.json. Variations are first-order (linear-response) propagations of Wan-Li 28-scheme moment shifts. Band rule (Wan-Li Ju): per scale take the larger of the two deviations (labelled 0p5 and 2: a factor of two for the scales, the quoted uncertainty ranges for C0_np, kappa_np and MuFtran (Ju, 2026-09-26)) from the central in each direction, clipped at zero; add the 14 scales (C0_np, kappa_np, MuBeam, MuSoft, MuFac, MuHard, MuCt, MuBF, NuBeam, NuSoft, MuF, MuR, MuRF, MuFtran) in quadrature, up and down separately. This is NOT the envelope over the 28 variations, which keeps only the largest scale per bin. apply_lambdas.theory_band() implements the rule.',
